...
public enum MetacriticStreamSelectors implements Extractable<Stream<Element>> {
    ...
    static class ScoreSupplier implements Supplier<Element> {
        ...
        public Element get() {
            ...
            var scoreIndex = from + "score:".length();
            int score = 0;
            while (Character.isDigit(content.charAt(scoreIndex))) {
                score = score * 10;
                score += content.charAt(scoreIndex) - '0';
                scoreIndex++;
            }
            index = scoreIndex;
            ...
            index = nameTo;
            final var paragraph = new Element("p");
            paragraph.appendChild(new Element("div").appendText(String.valueOf(score)));
            paragraph.appendChild(new Element("span").appendText(name));
            return paragraph;
        }
    }
